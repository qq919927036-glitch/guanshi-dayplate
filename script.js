...
const STEM_COLORS = {
  甲: { name: '青、绿色为主', note: '甲木日：青绿为主，黑蓝为辅。', swatch: 'linear-gradient(135deg,#2e7d5a 0 48%,#213d51 50% 100%)' },
  乙: { name: '青、绿色为主', note: '乙木日：青绿为主，黑蓝为辅。', swatch: 'linear-gradient(135deg,#2e7d5a 0 48%,#213d51 50% 100%)' },
  丙: { name: '红、紫色为主', note: '丙火日：红紫为主，青绿为辅。', swatch: 'linear-gradient(135deg,#b94e46 0 48%,#69446f 50% 100%)' },
  丁: { name: '红、紫色为主', note: '丁火日：红紫为主，青绿为辅。', swatch: 'linear-gradient(135deg,#b94e46 0 48%,#69446f 50% 100%)' },
  戊: { name: '黄、咖色为主', note: '戊土日：黄咖为主，红色为辅。', swatch: 'linear-gradient(135deg,#cba454 0 48%,#7e5232 50% 100%)' },
  己: { name: '黄、咖色为主', note: '己土日：黄咖为主，红色为辅。', swatch: 'linear-gradient(135deg,#cba454 0 48%,#7e5232 50% 100%)' },
  庚: { name: '白、金色为主', note: '庚金日：白金为主，黄咖为辅。', swatch: 'linear-gradient(135deg,#e7e2d1 0 48%,#b89c5a 50% 100%)' },
  辛: { name: '白、金色为主', note: '辛金日：白金为主，黄咖为辅。', swatch: 'linear-gradient(135deg,#e7e2d1 0 48%,#b89c5a 50% 100%)' },
  壬: { name: '黑、蓝色为主', note: '壬水日：黑蓝为主，白金为辅。', swatch: 'linear-gradient(135deg,#162c48 0 48%,#3d6f9a 50% 100%)' },
  癸: { name: '黑、蓝色为主', note: '癸水日：黑蓝为主，白金为辅。', swatch: 'linear-gradient(135deg,#162c48 0 48%,#3d6f9a 50% 100%)' },
};
...
const USE_CASES = [
  { id: '婚嫁', label: '婚嫁', terms: ['嫁娶', '纳采', '订盟', '会亲友'], avoid: ['安葬', '破土'] },
  { id: '入宅', label: '入宅', terms: ['入宅', '移徙', '安床'], avoid: ['安葬', '破土'] },
  { id: '开工', label: '开工', terms: ['修造', '动土', '竖柱', '上梁'], avoid: ['安葬'] },
  { id: '开业', label: '开业', terms: ['开市', '交易', '立券', '纳财'], avoid: ['安葬', '破土'] },
  { id: '签约', label: '签约', terms: ['交易', '立券', '纳财', '订盟'], avoid: ['词讼'] },
  { id: '出行', label: '出行', terms: ['出行', '赴任', '求嗣', '祈福'], avoid: ['安葬'] },
];
...
function getDayData(iso) {
  ...
  const data = {
    iso, year, month, day, solar, lunar, dayPillar, dayStem, deity, deityLuck, officer, officerLuck: officerQuality(officer), xiu, xiuLuck,
    ...
    dayChongDesc: lunar.getDayChongDesc(), dayChongZodiac: lunar.getDayChongShengXiao(), daySha: lunar.getDaySha(), tai: lunar.getDayPositionTai(), color: STEM_COLORS[dayStem] || STEM_COLORS.戊,
    lunarLabel: `农历${lunar.getMonthInChinese()}月${lunar.getDayInChinese()}`, fullPillars: `${lunar.getYearInGanZhi()}年 · ${lunar.getMonthInGanZhi()}月 · ${dayPillar}日`, weekday: `星期${solar.getWeekInChinese()}`,
  };
...
function directVerdict(data) {
  ...
  const base = `日值${data.deity}${isYellowDay(data) ? '黄道' : '黑道'}；${data.officer}日，${data.xiu}为${data.xiuLuck}宿。`;
  if (score === 3) return `${base} 三家同吉，可列为重要用事的优先候选。`;
  if (score === 2) return `${base} 三家两吉，取用时先避冲煞，再配合黄道吉时。`;
  if (score === 1) return `${base} 三家条件不齐，婚嫁、入宅、开工、安葬不作首选。`;
  return `${base} 三家不占优，重大用事改取他日；定期不能移时，只取黄道吉时。`;
}
...
function renderUseCaseTabs() {
  ...
  USE_CASES.forEach((useCase) => {
    const button = document.createElement('button');
    button.type = 'button';
    button.className = `usecase-tab${useCase.id === selectedUseCase ? ' is-selected' : ''}`;
    button.textContent = useCase.label;
    button.addEventListener('click', () => { selectedUseCase = useCase.id; renderUseCaseTabs(); renderUseCaseVerdict(); renderHours(getHourIndex(timeInput.value)); });
    root.appendChild(button);
  });
}
...
function renderUseCaseVerdict() {
  ...
  setText('#usecase-verdict', `${yiText}${jiText} ${DAY_DEITY_GUIDANCE[currentData.deity] || ''}`);
  setText('#usecase-timing', `择时：${timeText}优先；时段内仍以“时宜”与时冲逐项复核。`);
}
...
function renderDay(data) {
  ...
  setText('#zhi-xing', `${data.officer}日`); setText('#zhi-xing-note', OFFICER_GUIDANCE[data.officer] || '依月令起建，逐日轮转。'); setText('#xiu', data.xiu); setText('#xiu-note', `星宿${data.xiuLuck}；演禽宜忌作第二层复核。`); setText('#na-yin', data.lunar.getDayNaYin()); setText('#xun-kong', `${data.xun} · 空亡${data.xunKong}`); setText('#wutu-time', `太阳${data.wutu.sun.join('')} · 太阴${data.wutu.moon.join('')}`);
  ...
  setText('#wear-colors', data.color.name); setText('#color-note', data.color.note); document.querySelector('.swatch').style.background = data.color.swatch;
  ...
  setText('#engine-state', `已核定 ${data.iso}：${data.fullPillars} · ${data.deity}${yellow ? '黄道' : '黑道'} · ${data.xiu}。`); document.querySelector('#engine-state').className = 'console-note';
}
...
function updateDay({ preserveMonth = false } = {}) {
  ...
  } catch (error) { currentData = null; const note = document.querySelector('#engine-state'); note.textContent = error.message; note.className = 'console-note is-error'; setText('#direct-verdict', '日课引擎未就绪，暂不生成日时结论。'); }
}
